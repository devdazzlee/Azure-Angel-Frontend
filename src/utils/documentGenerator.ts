...
export async function generatePDF(
    ...
    switch (item.type) {
      ...
      case 'table':
        if (item.tableData) {
          ...
          const numericHeaderPattern =
            /estimated|actual|amount|price|volume|revenue|variance|value|total|projected|metric/i;
          const columnStyles: Record<number, { halign: 'left' | 'right' | 'center' }> = {};
          item.tableData.headers.forEach((header, index) => {
            if (index > 0 && numericHeaderPattern.test(header)) {
          ...
          autoTable(doc, {
            startY: yPosition,
            head: [item.tableData.headers.map((h) => cleanText(h))],
            body: item.tableData.rows.map((row) => row.map((cell) => cleanText(cell))),
            theme: 'striped',
            headStyles: {
  ...
  for (let page = 1; page <= totalPages; page++) {
    ...
    doc.text('Generated by Angel Business Assistant', margin, pageHeight - 10);
    doc.text(`Page ${page} of ${totalPages}`, pageWidth - margin, pageHeight - 10, {
      align: 'right',
    });
  }
